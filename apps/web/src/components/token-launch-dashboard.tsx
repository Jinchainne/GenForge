...
export function TokenLaunchDashboard({
  ...
  const [deployState, setDeployState] = useState<{
    ...
    message:
      tokenConfigIssues.length === 0
        ? "Settlement factory is configured. Connect a wallet from the top bar to submit."
        : "Configure the GenLayer settlement token factory before recording a settlement token.",
  });
  ...
  const deploymentRequest = useMemo(
    () => ({
      ...
      evidenceBoundary:
        "Valid only when the GenLayer factory returns a real transaction receipt.",
    }),
    [form, wallet.address],
  );
    ...
    <section className="token-shell">
      <div className="submission-layout">
        ...
        <section className="panel hero-panel workbench-rail">
          ...
          <h2>Wallet-signed settlement registry</h2>
          <p>
            Record a credit or settlement token request for an accepted trade
            case. Requires a real factory and receipt.
          </p>
